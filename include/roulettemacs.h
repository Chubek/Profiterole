...
#define TOKEN_ConcatPeriod(TOKEN1, TOKEN2)                                     \
  __PRF_EVAL(__PRF_CONCAT, TOKEN1, TOKEN2)
#define TOKEN_ConcatLowdash(TOKEN1, TOKEN2)                                    \
  __PRF_EVAL(__PRF_PASTE, TOKEN1, _, TOKEN2)

#define DEFINE_Namespace(PARENT, CHILD)                                        \
  TOKEN_AsciiStr(TOKEN_ConcatPeriod(PARENT, CHILD))
...
#define DEFINE_Exception(NAME, NS)                                             \
  PyObjRef NAME = PyErr_NewException(NS, NULL, NULL)
  ...
  TYY *TOKEN_ConcatLowdash(NAME, SuffixArray)[] =                              \
      {                                                                        \
          ...
          NULL,                                                                \
                                 ...
                                 **TOKEN_ConcatLowdash(NAME, SuffixPtr) =      \
                                     &TOKEN_ConcatLowdash(NAME,                \
                                                          SuffixArray)[0],     \
                                 *TOKEN_ConcatLowdash(NAME, SuffixTemp)

#define REFLIST_ApplyXrs(NAME, APPLYFN)                                        \
  do {                                                                         \
    TOKEN_ConcatLowdash(NAME, SuffixTemp) =                                    \
        *TOKEN_ConcatLowdash(NAME, SuffixPtr)++;                               \
    APPLYFN(TOKEN_ConcatLowdash(NAME, SuffixTemp));                            \
